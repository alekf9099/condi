...
**셀렉터 탭** — 요소를 고르면 셀렉터가 등록되고 **`보임` 검증 단계가 함께 생깁니다.**
고른 뒤 바로 실행하면 그 요소가 검증됩니다. 다른 동작이 필요하면 각 셀렉터 옆의
`클릭` · `보임` · `없음` 버튼으로 단계를 더 붙이면 됩니다.

역할을 나누면 이렇습니다 — **녹화는 동작**(클릭·입력), **셀렉터 지정은 검증**입니다.

설정이 아직 없어도 됩니다. 고른 셀렉터를 담을 설정이 없으면
**지금 보고 있는 페이지**를 타겟으로 자동 생성됩니다(사이트 루트가 아니라
그 페이지여야 실행할 때 화면을 떠나지 않습니다). `요소 선택 시작`을 누르고 대상 페이지에서 요소를 클릭하면
안정적인 셀렉터(`data-testid` > `id` > `name` > `aria-label` > 최단 CSS 경로)가
자동 생성되어 설정의 `selectors`에 추가됩니다. <kbd>Esc</kbd>로 취소합니다.

고르는 동안에는 페이지가 **어떤 입력에도 반응하지 않습니다.** 사이드바 메뉴처럼
`mousedown`에서 이동하는 요소도 화면을 넘기지 않고 그대로 담을 수 있습니다.
